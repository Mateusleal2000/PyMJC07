    ...
    def build_in_and_out(self):
        ...
        while do or (self.check_if_same(in_old, out_old)):
            ...
            while aux.head != None:
                ...
                diff = self.out_node_table[aux.head.to_string()].difference(self.flowgraph.deff(aux.head))
                self.in_node_table[aux.head.to_string()] = self.temp_list_to_set(self.flowgraph.use(aux.head)).union(diff)
                #Para todo sucessor de aux.head, devemos fazer a união de seus conjuntos de arestas
                succs: graph.NodeList = aux.head.succ()
                aux_succs = succs
                all_succs: set
                while aux_succs.head != None:
